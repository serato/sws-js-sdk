...
describe('Ecom Tests', function () {
  ...
  describe('Ecom URI Validation Tests', function () {
    ...
    it(`confirms URI used in 'getInvoiceNumber()' method without a user ID by returning a non-404 HTTP response`,
      function () {
        swsClient.userId = 0
        return swsClient.ecom.getInvoiceNumber(123, 456).then(
          () => Promise.reject(new Error('Expected non-2xx HTTP response code')),
          err => {
            expect(err.httpStatus).not.to.equal(404)
          }
    ...
    it(`confirms URI used in 'getInvoiceNumber()' method with a user ID by returning a non-404 HTTP response`,
      function () {
        swsClient.userId = 123
        return swsClient.ecom.getInvoiceNumber(123, 456).then(
          () => Promise.reject(new Error('Expected non-2xx HTTP response code')),
